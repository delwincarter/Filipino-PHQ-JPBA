...
All values reported in the manuscript tables (fit indices, loadings, reliability estimates, and correlations) are regenerated directly from this workflow.  
The tables here serve as computational verification tables; the published manuscript tables present the same values in typeset form.
...
The analytic dataset is not included in this repository at present because it is under restricted access. Pending permission from the dataset owner, a de-identified research file will be deposited in a public repository and linked here.
